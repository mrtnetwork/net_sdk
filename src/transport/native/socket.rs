...
use crate::{
    ...
    types::{
        ...
        request::{NetRequest, NetSocketRequestSend},
        response::{
            NetResponseKind, NetSocketResponseOk, NetStreamResponse, NetStreamResponseData,
...
impl Transport for SocketTransport {
    ...
    async fn do_request<'a>(
        ...
        let _ = match socket_requset {
            crate::types::request::NetSocketRequest::Subscribe => self.subscribe().await?,
            crate::types::request::NetSocketRequest::Unsubscribe => self.unsubscribe().await?,
            crate::types::request::NetSocketRequest::Send(socket_request_send) => {
                self.send(socket_request_send).await?
            }
        };
        Ok(NetResponseKind::Socket(NetSocketResponseOk))
    }
                ...
                match rx.recv().await {
                    Ok(msg) => match msg {
                        Ok(data) => match data {
                            ...
                            None => {
                                callback(NetResponseKind::Stream(NetStreamResponse::Close(None)));
                                break;
                            }
                        },
                        Err(err) => {
